...
4. ✅ **Bildeopplasting implementert.** Editorens bilde-felt (retter + oppskrifter) har forhåndsvisning + filopplasting + «fjern» + redigerbar URL. Nye filer: `src/lib/storage-admin.js` (`uploadImage(prefix, file, onProgress)` — resumable, fremdrift), `src/lib/upload-validate.js` (`validateImageFile`/`safeFileName` — rene, testet: 10 Node-tester). Laster opp til `media/<col>/<id>/<tid>-<navn>`, matcher `storage.rules` (image/*, <25 MB), skriver nedlastings-URL til `item.image` og lagres som en vanlig felt-endring via editoren. Verifisert: render (forhåndsvisning/fil-input/URL/fjern), interaksjon, validering. **Åpent/uverifisert:** selve opplastingen krever admin-innlogging (test live) — sjekk at bildet dukker opp i medieblokken etter lagring + reload. Vurder også sletting av gammelt Storage-objekt ved erstatning (ryddes ikke i dag).

## 6. Åpne punkter / advarsler
- **Matfaglig-allergener: AVKLART 20. aug 2026 (Torkil, Matfaglig).** De 7 rettene (`p27`, `p28`, `d58`, `p33`, `p34`, `p35`, `p36`) er avklart og `alReview` er fjernet i `seed.json`. Svar: croissant-deig har egg → `egg` lagt til på p27/p28/d58; bagel-deig har verken sesam eller melk; pesto = melk (ingen nøtter); salami = gluten+melk; chickenparm smøresalat = egg+melk — alt dekket av eksisterende `gluten/egg/melk` på bagel-rettene. **NB:** endringen er i `seed.json`; Firestore (live) må oppdateres — enten via admin-editoren (fjern haken «Allergener uavklart» + sett allergener) eller ny import. `alReview`-mekanismen beholdes for framtidige avklaringer.
